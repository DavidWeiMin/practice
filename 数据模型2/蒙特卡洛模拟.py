...
plt.xlabel('破产银行数量')
plt.ylabel('频数')
fig1.savefig('D:/Documents/GitHub/practice/数据模型2/基础违约分布')

fig2 = plt.figure(dpi=300)
plt.hist(fundamental_number,bins=max(contagion_number) - min(contagion_number))
plt.xlabel('破产银行数量')
plt.ylabel('频数')
fig2.savefig('D:/Documents/GitHub/practice/数据模型2/传染违约分布')

fig3 = plt.figure(dpi=300)
plt.hist(fundamental_number,bins=max(default_number) - min(default_number))
plt.xlabel('破产银行数量')
plt.ylabel('频数')
fig3.savefig('D:/Documents/GitHub/practice/数据模型2/总违约分布')

# 银行网络结构图（无向）
fig4 = plt.figure(dpi=300)
nx.draw(G,with_labels=True,font_size=6,node_size=80)
plt.title('银行网络结构图（无向）')
fig4.savefig('D:/Documents/GitHub/practice/数据模型2/银行网络结构图（无向）')

fig5 = plt.figure(dpi=300)
plt.hist(LGD_avg)
plt.xlabel('平均违约损失率')
plt.ylabel('频数')
fig5.savefig('D:/Documents/GitHub/practice/数据模型2/平均违约损失率分布')
